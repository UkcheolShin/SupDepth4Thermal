...
class UnetAdaptiveBins(nn.Module):
    def __init__(self, backend, n_bins=100, min_val=0.1, max_val=10, norm='linear'):
        ...
        self.conv_out = nn.Sequential(nn.Conv2d(128, n_bins, kernel_size=1, stride=1, padding=0),
                                      nn.Softmax(dim=1))
        self.features = [24, 40, 64, 176, 2048]

    def forward(self, x, **kwargs):
        features = self.encoder(x)
        feats = [features[4], features[5], features[6], features[8], features[11]]
        unet_out = self.decoder(feats, **kwargs)
    ...
    def forward_depth(self, feats, **kwargs):
        unet_out = self.decoder(feats, **kwargs)
        bin_widths_normed, range_attention_maps = self.adaptive_bins_layer(unet_out)
        out = self.conv_out(range_attention_maps)
