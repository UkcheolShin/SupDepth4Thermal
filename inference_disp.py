...
def main():
    ...
    for i, img_file in enumerate(tqdm(image_files)):
        ...
        tgt_img_right  = load_as_float_img(img_file[:-20]+'/right/'+img_file[-14:]) # HWC
        tensor_img = inference_transform([tgt_img_left] + [tgt_img_right], None, None)

        tgt_left_in = tensor_img[0]['img_in'][0].cuda()
        tgt_right_in = tensor_img[0]['img_in'][1].cuda()

        pred_disp = model.inference_disp(tgt_left_in.unsqueeze(0).cuda(), tgt_right_in.unsqueeze(0).cuda())

        # save prediction
        if  args.save_dir != ' ':
            img_vis = visualize_image(tgt_left_in, flag_np=True).transpose(1,2,0)
            img_vis2 = visualize_image(tgt_right_in, flag_np=True).transpose(1,2,0)
            pred_disp_vis = visualize_disp_as_numpy(pred_disp.squeeze(), 'jet')

            png_path = osp.join(save_dir_all, "{}.png".format(filename))
            stack = cv2.cvtColor(np.concatenate((img_vis, img_vis2, pred_disp_vis), axis=0), cv2.COLOR_RGB2BGR)
